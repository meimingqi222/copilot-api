...
- [~] T5.2 [已批准 2026-07-15] 按 T5.1 批准后的设计实施（⏸️ 暂停：当前版本可用，后续再进一步重构）
  ...
  - [x] T5.2.2 内存模型翻转（admin 直写 connections + `state.accounts` 删除 + getter/setter 删除，可拆 a/b/c/d）— **完成**：a/b/c 子批次已提交（connection mutation helpers + state.accounts 改为派生缓存 + 生产 mutation 站点改 connection helpers），**d 子批次完成内存模型翻转**：`state.accounts` 和 `state.activeAccountIndex` 从 `State` 接口删除，`deriveAccountsFromConnections()`/`syncAccountsFromConnections()` 删除，~130 个 `state.accounts` 读取改为 `listAccounts()`/`getAccount()`，~25 个 `state.activeAccountIndex` 读取改为 `getActiveAccountId()` helper。`saveAccountsUnlocked`/`flushAccountsOnShutdown`/`refreshCopilotToken` 改为直接调用 `saveProviderConnections(listProviderConnections())`（绕过 `persistenceEnabled` flag 以兼容测试隔离）。`loadAccountsUnlocked` 新增 `normalizeAllConnectionRuntimeFields()` 镜像原 `normalizeAccountRuntimeFields` 逻辑（cooldownUntil 过期重置 + lastRateLimit 清除）。`cpa-import.ts:removeDuplicateAccount` 新增 `removeProviderConnection(existing.id)` 调用确保 overwrite 时从 connection 删除旧账号。`saveAccountsUnlocked` 新增 `connectionsDiskHasData()` 检查拒绝空快照覆盖有数据的 provider-connections.json。grep 验收：`state.accounts`/`state.activeAccountIndex` 在 `src/`+`tests/` 仅剩注释引用（0 代码引用）。
  - [x] T5.2.3 `RouteTarget.account` 特例删除 + protocols 层 `requireTargetAccount` 删除
  - [~] T5.2.4 清理与文档（删除 account-store/account-file-store/account-adapter 等无引用文件 + 更新 AGENTS.md）— **仍部分完成**：7 个 `account-*.ts` 文件均仍有活跃引用（`account-store.ts` 被 21 处导入含 token 刷新/配额刷新逻辑无等效、`account-availability.ts` 被 9 处导入、`account-adapter.ts` 被 7 处导入含 `applyConnectionPatchToAccount` 无等效、`account-selection.ts` 被 3 处导入含 `getActiveAccount` 无等效、`account-file-store.ts`/`account-legacy-migrator.ts` 仍用于 accounts.json 迁移检测、`account-diagnostics.ts` 被 availability 导入）。需先迁移功能到 `provider-connections/` 才能删除。⏸️ 暂停
...
| 2026-07-16 | T5.2.2d | ✅ 完成 | **内存模型翻转**——消除 `state.accounts`/`state.activeAccountIndex` 字段，完成 T5.2.2 设计目标的"消除双模型"。改动：(1) `state.ts` 删除 `accounts`/`activeAccountIndex` 字段 + `deriveAccountsFromConnections()`/`syncAccountsFromConnections()` 函数 + `Account` 类型 import；(2) `accounts.ts` 新增 `listAccounts()`/`getAccount(id)`/`addAccount(account)` helpers（从 `listProviderConnections()` 经 `readAccountLegacyMetadata` 过滤 + `connectionToAccount` 映射）；(3) ~40 个源文件 + ~20 个测试文件的 ~130 个 `state.accounts` 读取改为 `listAccounts()`/`getAccount()`，~25 个 `state.activeAccountIndex` 读取改为 `getActiveAccountId()` helper（admin/api/accounts.ts）；(4) `account-store.ts` 的 `saveAccountsUnlocked`/`flushAccountsOnShutdown`/`refreshCopilotToken` 改为直接调用 `saveProviderConnections(listProviderConnections())`（绕过 `persistenceEnabled` flag 以兼容 `__resetProviderConnectionsForTest()` 测试隔离）；(5) `loadAccountsUnlocked` 新增 `normalizeAllConnectionRuntimeFields()` 镜像原 `normalizeAccountRuntimeFields` 逻辑（cooldownUntil 过期重置 + lastRateLimit 清除，同时处理 credential.cooldownUntil 和 metadata.cooldownUntil）；(6) `cpa-import.ts:removeDuplicateAccount` 新增 `removeProviderConnection(existing.id)` 调用确保 overwrite 时从 connection 删除旧账号；(7) `saveAccountsUnlocked` 新增 `connectionsDiskHasData()` 检查拒绝空快照覆盖有数据的 provider-connections.json；(8) `utils.ts:refreshModelsForAccount` 新增 `getProviderConnection(account.id)` guard 防止后台异步 refresh 在测试 cleanup 后重新添加已删除的 account（修复 `provider-defaults.test.ts` flaky test——`oauth-smoke.test.ts` 的 `refreshModelsForAccount` 后台调用在 `afterEach` cleanup 后 `upsertProviderConnection` 重新添加 xai-export account）；(9) `create-chat-completions.test.ts` 的 `afterEach` 修复 `state.providerDefaults` 恢复用 `structuredClone` 而非引用赋值。**grep 验收**：`state.accounts`/`state.activeAccountIndex` 在 `src/`（24+2 匹配）+ `tests/`（4+0 匹配）仅剩注释引用，0 代码引用。三件套全绿：typecheck ✓ / lint:all ✓ / 558 pass / 2 skip / 0 fail（10 次连续运行全绿）。 |
| 2026-07-16 | T5.2.4 | 🔄 仍部分完成 | T5.2.2d 完成后重新评估：7 个 `account-*.ts` 文件**均仍有活跃引用**，不能直接删除。`account-store.ts`（21 处导入，含 `refreshCopilotToken`/`scheduleQuotaRefresh`/`refreshQuotaForAccount`/`cancelTokenRefreshTimer` 无等效）、`account-availability.ts`（9 处导入，`getAccountAvailability`/`syncLegacyExhaustedState`/`getMinimumCooldownSeconds` 无等效）、`account-adapter.ts`（7 处导入，`applyConnectionPatchToAccount`/`AccountConnectionPatch` 无等效）、`account-selection.ts`（3 处导入，`getActiveAccount` 无等效）、`account-file-store.ts`/`account-legacy-migrator.ts`（仍用于 accounts.json 迁移检测）、`account-diagnostics.ts`（被 availability 导入）。需先在 `provider-connections/` 实现等效功能（connection 级别选择器、token 刷新调度、配额刷新、诊断快照、admin patch 反向映射）才能删除。⏸️ 暂停，后续作为独立批次。 |
